...
"""
    ...
    def initializeDICOMDatabase(self):
        ...
        database_path = os.path.join(slicer.util.mainWindow().GetProjectBasePath(),"Resources","Database")
        # DICOMUtils.openDatabase(database_path)
        # slicer.dicomDatabase.initializeDatabase(database_path)
        setting = qt.QSettings()
        setting.setValue(slicer.dicomDatabaseDirectorySettingsKey, database_path)
        print("slicer.dicomDatabaseDirectorySettingsKey:",slicer.dicomDatabaseDirectorySettingsKey)
...
class DICOMFileDialog:
    ...
    def dropEvent(self):
        ...
        print("dropEvent",self.directoriesToAdd)
        for path in self.directoriesToAdd:
            util.getModuleWidget("JPACS")._on_load_from_folder(path)
        #slicer.util.selectModule('DICOM')
...
class DICOMLoadingByDragAndDropEventFilter(qt.QWidget):
    ...
    def eventFilter(self, object, event):
        ...
        if event.type() == qt.QEvent.DragEnter:
            print("drag enter in dicom")
            self.dragEnterEvent(event)
            return True
        if event.type() == qt.QEvent.Drop:
            print("drag drop in dicom")
