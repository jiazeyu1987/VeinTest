...
class SlicerDICOMBrowser(VTKObservationMixin, qt.QWidget):
    ...
    def setup(self, showPreview=False):
        ...
        self.uncheckAllButton.setMinimumHeight(31)

        self.actionButtonLayout.addStretch(0.05)

        self.examineButton = qt.QPushButton('检查')
        ...
        self.examineButton.setMinimumHeight(31)

        self.loadButton = qt.QPushButton('加载')
        ...
        # self.exitButton = qt.QPushButton('退出')
        # self.exitButton.setSizePolicy(qt.QSizePolicy.Expanding, qt.QSizePolicy.Fixed)
        # self.exitButton.toolTip = 'Load selected items into the scene'
        # self.actionButtonLayout.addWidget(self.exitButton)
        # self.exitButton.connect('clicked()', self.on_exit)
        # self.exitButton.setMinimumHeight(31)

        self.actionButtonLayout.addStretch(0.05)

        self.advancedViewButton = qt.QCheckBox('高级')
    ...
    def on_exit(self):
        util.getModuleWidget("JMeasure").on_exit()
    ...
    def getLoadablesFromFileLists(self, fileLists):
        ...
        selectedPlugins = []
        disabledPlugins_str = util.getjson("disabledPlugins","")
        disabledPlugins=[]
        if disabledPlugins_str != "":
            disabledPlugins = disabledPlugins_str.split(",")
        if settings.contains('DICOM/disabledPlugins/size'):
            ...
            for pluginClass in slicer.modules.dicomPlugins:
                if pluginClass not in disabledPlugins:
                    print("selectedPlugins",pluginClass)
        ...
        if missingFileCount < allFileCount:
            ...
            def progressCallback(progressDialog, progressLabel, progressValue):
                ...
                if progressValue==100:
                    progressValue = 99
                util.send_event_str(util.ProgressValue,progressValue)
                return False

            loadablesByPlugin, loadEnabled = DICOMLib.getLoadablesFromFileLists(fileLists, selectedPlugins, messages,
                                                                                lambda progressLabel, progressValue, progressDialog=None: progressCallback(progressDialog, progressLabel, progressValue),
                                                                                self.pluginInstances)
    ...
    def loadCheckedLoadables(self):
        ...
        if len(slicer.dicomDatabase.patients()) == 0:
            return
        if self.advancedViewButton.checkState() == 0:
            self.examineForLoading()
    ...
    def proceedWithReferencedLoadablesSelection(self):
        ...
        slicer.app.ioManager().emitNewFileLoadedCustom(loadedFileParameters)
