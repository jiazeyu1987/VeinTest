...
class SelfTestsWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        self.reloadALl = qt.QPushButton("ReloadALl")
        self.testList.layout().addWidget(self.reloadALl)
        self.reloadALl.connect('clicked()', self.onReloadALl)
    ...
    def init(self,datas):
        self.selfTests = datas
        self.logic.selfTests = datas
        testKeys = sorted(self.selfTests.keys())
        slicer.selfTests =datas
    ...
    def enter(self):
        self.init(slicer.selfTests)

    def onReloadALl(self):
        for test in self.selfTests:
            util.getModuleWidget(test).onReload()
        self.onReload()
