...
class EndoscopyWidget(ScriptedLoadableModuleWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.transform = None
        self.path = None
        self.flythroughFormLayout = None
        self.camera = None
        self.skip = 0
    ...
    def setup(self):
        ...
        self.flythroughFormLayout = flythroughFormLayout
        # Frame slider
        ...
        flythroughFormLayout.addRow("帧数:", frameSlider)
        ...
        flythroughFormLayout.addRow("帧数忽略:", frameSkipSlider)
        ...
        frameDelaySlider.maximum = 200
        frameDelaySlider.suffix = " ms"
        frameDelaySlider.value = 100
        flythroughFormLayout.addRow("帧数延迟:", frameDelaySlider)
        ...
        viewAngleSlider.maximum = 300
        viewAngleSlider.value = 100
        flythroughFormLayout.addRow("观察角度:", viewAngleSlider)

        # Play button
        playButton = qt.QPushButton("开始")
    ...
    def onPlayButtonToggled(self, checked):
        if checked:
            self.timer.start()
            self.playButton.text = "结束"
            self.show_markups_and_models(False)
        else:
            self.timer.stop()
            self.playButton.text = "开始"
            self.show_markups_and_models(True)

    def show_markups_and_models(self,boolval):
        import slicer.util as util
        if not boolval:
            markups = util.getNodesByClass(util.vtkMRMLMarkupsNode)
            models = util.getNodesByClass(util.vtkMRMLModelNode)
            allist = markups+models
            for node in allist:
                display_node = util.GetDisplayNode(node)
                vis = display_node.GetVisibility()
                if vis:
                    display_node.SetVisibility(False)
                    self.paras[display_node.GetID()] = "1"
        else:
            for key in self.paras:
                node = util.GetNodeByID(key)
                if node:
