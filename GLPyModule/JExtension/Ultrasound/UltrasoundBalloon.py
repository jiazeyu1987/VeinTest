...
class UltrasoundBalloon(ScriptedLoadableModule):

  def __init__(self, parent):
    ScriptedLoadableModule.__init__(self, parent)
    self.parent.title = "UltrasoundBalloon"  # TODO: make this more human readable by adding spaces
...
class UltrasoundBalloonWidget(JBaseExtensionWidget):
  ...
  def setup(self):
    super().setup()
    print("UltrasoundBalloonWidget setup")
  ...
  def init_epack_list(self):
    for i in range(len(self.title_list)):
      template1 = slicer.util.loadUI(self.resourcePath("UI/BalloonItem.ui"))
      template1ui = slicer.util.childWidgetVariables(template1)
      template = BalloonItem(template1ui, self.title_list[i], self.content_list[i])
