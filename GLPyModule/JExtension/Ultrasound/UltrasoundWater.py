...
class UltrasoundWater(ScriptedLoadableModule):

  def __init__(self, parent):
    ScriptedLoadableModule.__init__(self, parent)
    self.parent.title = "UltrasoundWater"  # TODO: make this more human readable by adding spaces
...
class UltrasoundWaterWidget(JBaseExtensionWidget):
  ...
  def setup(self):
    super().setup()
    print("UltrasoundWaterWidget setup")

  def enter(self):
    self.init_once()
    self.addEvent(True)
    util.save_global_value("water_pack", True)
  ...
  def init_epack_list(self):
    for i in range(len(self.title_list)):
      template1 = slicer.util.loadUI(self.resourcePath("UI/WaterItem.ui"))
      template1ui = slicer.util.childWidgetVariables(template1)
      template = WaterItem(template1ui, self.title_list[i], self.content_list[i])
