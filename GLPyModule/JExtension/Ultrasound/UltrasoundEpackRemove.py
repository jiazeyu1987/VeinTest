...
#
# UltrasoundEpackRemove
#

    
class UltrasoundEpackRemove(ScriptedLoadableModule):

  def __init__(self, parent):
    ScriptedLoadableModule.__init__(self, parent)
    self.parent.title = "UltrasoundEpackRemove"  # TODO: make this more human readable by adding spaces
...
class UltrasoundEpackRemoveWidget(JBaseExtensionWidget):
  init_once_flag = False
  def setup(self):
    super().setup()
    print("UltrasoundEpackRemoveWidget setup")

  def enter(self):
    self.init_once()
    self.addEvent(True)    
    util.save_global_value("arm_pack", False)
