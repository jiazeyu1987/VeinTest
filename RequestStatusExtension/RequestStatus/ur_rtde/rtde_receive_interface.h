...
#include <ur_rtde/pureray_receive_interface_api.h>
...
namespace ur_rtde
{
class RTDEReceiveInterface : public PurerayReceiveInterfaceAPI
{
 public:
...
RTDE_EXPORT void* createRTDEReceiveInstance(std::string hostname, double frequency = -1.0,
                                            std::vector<std::string> variables = {},
                                            bool verbose = false, bool use_upper_range_registers = false,
                                            int rt_priority = RT_PRIORITY_UNDEFINED);
